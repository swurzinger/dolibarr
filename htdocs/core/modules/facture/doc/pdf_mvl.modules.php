 ...
 */
class pdf_mvl extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "mvl";
		$this->description = $langs->trans('PDFMVLDescription');
	...
	public function drawPaymentsTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		// TODO: stw, increased font size, adjusted column widths (Amount -5, Type +5), removed Belegnummer-Referenz column header
		$pdf->SetFont('', '', $default_font_size - 3);
		$pdf->SetXY($tab3_posx, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("Payment"), 0, 'L', false);
		$pdf->SetXY($tab3_posx + 21, $tab3_top);
		$pdf->MultiCell(15, 3, $outputlangs->transnoentities("Amount"), 0, 'L', false);
		$pdf->SetXY($tab3_posx + 35, $tab3_top);
		$pdf->MultiCell(25, 3, $outputlangs->transnoentities("Type"), 0, 'L', false);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$pdf->MultiCell(15, 3, price((isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $obj->multicurrency_amount_ttc : $obj->amount_ttc, 0, $outputlangs), 0, 'L', false);
				$pdf->SetXY($tab3_posx + 35, $tab3_top + $y);
				$pdf->MultiCell(25, 3, $text, 0, 'L', false);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$pdf->SetXY($tab3_posx + 21, $tab3_top + $y);
				$pdf->MultiCell(15, 3, price($sign * ((isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $row->multicurrency_amount : $row->amount), 0, $outputlangs), 0, 'L', false);
				$pdf->SetXY($tab3_posx + 35, $tab3_top + $y);
				$oper = $outputlangs->transnoentitiesnoconv("PaymentTypeShort".$row->code);

				$pdf->MultiCell(25, 3, $oper, 0, 'L', false);
	...
	protected function drawInfoTable(&$pdf, $object, $posy, $outputlangs, $outputlangsbis)
	{
		...
		// TODO: stw, increased font size, added bold
		...
		if ($object->type != 2 && $object->cond_reglement_code) {
			...
			$posy = $pdf->GetY() + 6; // We need spaces for 2 lines payment conditions
		}
		...
		// Show no MwSt notice
		{
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->SetXY($this->marge_gauche, $posy);
			$text = 'Der Rechnungsbetrag enthält keine Mehrwertsteuer, da der Verein aufgrund seiner Gemeinnützigkeit nach §6 Abs. 1 Z 25 UStG 1994 umsatzsteuerbefreit ist.';
			$pdf->MultiCell($posxend - 5, 4, $text, 0, 'L');
		...
		if ($object->type != 2) {
			...
			// TODO: stw, hide when invoice has already been fully paid
			if (empty($paidOn) && empty($object->paye) && (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR')) {
				if ($object->fk_account > 0 || $object->fk_bank > 0 || getDolGlobalInt('FACTURE_RIB_NUMBER')) {
					...
					if (getDolGlobalString('INVOICE_ADD_EPC_QR_CODE') == 'bottom') {
						...
						// TODO: stw, hide EPC QR info text (translation doesn't work ?!)
//						$pdf->SetXY($qrPosX + 30, $qrPosY + 5);
//						$pdf->SetFont('', '', $default_font_size - 4);
...
//						$posy = $pdf->GetY() + 2;
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		$posy += 3;
		// TODO: stw, increased header text font size (invoice date, ...) and $posy vertical spacing
		$pdf->SetFont('', '', $default_font_size - 1);
		...
		// TODO: stw
		// Leistungszeitraum
		$deliveryDate = $this->getExtrafieldContent($object, "delivery_date");
		if (!empty($deliveryDate)) {
			$posy += 4;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$title = "Liefer-/Leistungszeitraum";
			$pdf->MultiCell($w, 3, $title." : ".$deliveryDate, '', 'R');
		}
		...
		if (!empty($object->thirdparty->tva_intra)) {
			...
			$pdf->SetTextColor(0, 0, 60);
			$title = "Ihre UID";
			$pdf->MultiCell($w, 3, $title." : ".$object->thirdparty->tva_intra, '', 'R');
		}
		...
		if ($showaddress) {
			...
			if (getDolGlobalString('MAIN_INVERT_SENDER_RECIPIENT')) {
				// TODO: stw, adjust "from" address box
