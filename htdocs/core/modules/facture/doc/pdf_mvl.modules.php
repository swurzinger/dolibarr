...
class pdf_mvl extends ModelePDFFactures
{
	...
	public function drawPaymentsTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell(15, 3, $outputlangs->transnoentities("Amount"), 0, 'L', 0);
		$pdf->SetXY($tab3_posx + 35, $tab3_top);
		$pdf->MultiCell(25, 3, $outputlangs->transnoentities("Type"), 0, 'L', 0);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$pdf->MultiCell(15, 3, price((isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $obj->multicurrency_amount_ttc : $obj->amount_ttc, 0, $outputlangs), 0, 'L', 0);
				$pdf->SetXY($tab3_posx + 35, $tab3_top + $y);
				$pdf->MultiCell(25, 3, $text, 0, 'L', 0);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$pdf->SetXY($tab3_posx + 21, $tab3_top + $y);
				$pdf->MultiCell(15, 3, price($sign * ((isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $row->multicurrency_amount : $row->amount), 0, $outputlangs), 0, 'L', 0);
				$pdf->SetXY($tab3_posx + 35, $tab3_top + $y);
				$oper = $outputlangs->transnoentitiesnoconv("PaymentTypeShort".$row->code);

				$pdf->MultiCell(25, 3, $oper, 0, 'L', 0);
